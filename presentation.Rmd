...
## {data-background="img/excited_MSS0COPq80x68.gif"}
...
<img src="http://i.imgur.com/4UsFlzv.gif" height=200px />
</div>
<div class = "fragment">
### in R
...
<div class = "fragment">
```{r a06_teil2_skizze, fig.height=5}
ggplot(data = NULL, aes(x = -4:4)) + 
  stat_function(fun = dnorm) +
  ...
  annotate("label", x = .2,  y = .2, label = "0.191") +
  annotate("label", x = -.2, y = .2, label = "0.5") +
  scale_x_continuous(breaks = seq(-4, 4, 1), minor_breaks = seq(-4, 4, .5)) + 
  labs(x = "x", y = expression(P(x)), title = expression(N(0, 1))) + 
  theme(panel.grid.major = element_line(size = 0.1), 
        panel.grid.minor = element_line(linetype = "blank"),
...
# Aufgabe 8 <br /><br />$\chi^2$, Cramer's $V$<br /><small>\*sigh\* …und $C$</small>
...
<div class = "fragment">
\[\begin{aligned}
\text{Cramer's}\ V & = \sqrt{\frac{\chi^2}{n \cdot \operatorname{min}(r-1, c-1)}} \\
                  & = \sqrt{\frac{0.544}{246}} \\
...
<div class = "fragment big">NO WAIT!<br />Was ist das Problem?</span>
...
<div class = "fragment">…Außer ihr seid euch ganz sicher</div>
...
> - 2. Die Gerade mit den geringsten Resiudenquadraten.  
...
ggplot(zombo, aes(x = Skill_03, y = Skill_08)) +
  geom_ribbon(aes(x = seq(1, 2, 0.009434067), ymin = 2.412,
                  ymax = seq(2.412, 3.119, 0.00673333333)),
              fill = "blue", alpha = 0.3) +
  ...
  annotate("text", x = 9.75, y = 8.2, label = "y'", color = "red", size = 8) +
  annotate("text", x = 0.25, y = 5.1, label = expression("P(" ~ bar(x) ~ ";" ~ bar(y) ~ ")"), 
           color = "darkgreen", size = 8, parse = T) +
...
$$
r = \frac{\frac 1n \sum^n_{i=1} \left(x_i - \bar{x}\right) \cdot \left(y_i - \bar{y}\right)}
        {\sqrt{\frac 1n \sum^n_{i=1} \left(x_i - \bar{x}\right)^2} \cdot
        \sqrt{\frac 1n \sum^n_{i=1} \left(y_i - \bar{y}\right)^2}}
$$
</div>
...
Im Kontext der Kovarianz sind die Quadranten gemeint, die sich aus $\bar{x}$ und $\bar{y}$ ergeben
